    ...
    <div className={styles.panel}>
      ...
      {result.definition && (
        <section className={styles.section}>
          ...
          <p className={styles.sectionBody} dangerouslySetInnerHTML={{ __html: result.definition.content }} />
        </section>
      )}
      ...
      {result.explanation && (
        <section className={styles.section}>
          ...
          <p className={styles.sectionBody}>{result.explanation.content}</p>
          {result.explanation.formula && (
            <div className={styles.formulaBox}>
              <code className={styles.formula}>{result.explanation.formula}</code>
              {result.explanation.formulaNote && <p className={styles.formulaNote}>{result.explanation.formulaNote}</p>}
            </div>
          )}
        </section>
      )}
        ...
        <section className={styles.section}>
          ...
          <ul className={styles.points}>
            {result.keyPoints.map((kp, i) => (
              <li key={i}><span className={styles.bullet} /><span><strong>{kp.label}</strong> {kp.text}</span></li>
            ))}
          </ul>
        </section>
      )}
      ...
      {sources.length > 0 && (
        <div className={styles.sources}>
          ...
          <div className={styles.sourceChips}>
            {visibleSources.map((s, i) => <span key={i} className={styles.chip}>{s}</span>)}
            {hiddenCount > 0 && (
